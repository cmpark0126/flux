...
from flux.util import configs, load_ae, load_clip, load_flow_model, load_t5, save_image_without_nsfw_check
...
class FluxBenchmark:
    ...
    def setup_model(self):
        ...
        self.t5 = load_t5(self.device, max_length=256 if self.options.name == "flux-schnell" else 512)
    ...
    def _single_run(self, seed: int | None = None) -> float:
        ...
        with torch.inference_mode():
            ...
            x = denoise(self.model, **inp, timesteps=timesteps, guidance=self.options.guidance)
    ...
    def run_benchmark(self):
        ...
        if self.options.save_results:
            ...
            result_file = Path(self.options.output_dir) / f"benchmark_results_{timestamp}.json"
